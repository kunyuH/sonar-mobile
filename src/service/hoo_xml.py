...
def find(root,selector_str,is_out_tree=False):
    try:
...
def click(attrib):
    dpi_ratio = GCT().get('dpi_ratio',1)

    x = int(attrib['x']) * dpi_ratio
    y = int(attrib['y']) * dpi_ratio
    width = int(attrib['width']) * dpi_ratio
    height = int(attrib['height']) * dpi_ratio
