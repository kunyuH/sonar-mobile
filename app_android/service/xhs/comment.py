...
from .common import is_user_page
from ....src.utils.tools import (parse_chinese_time,date_to_timestamp,timestamp_to_date,check_end,on,out_info,
                                 run_sel,out_success,getNoteIdByUrl,send, getUrl, getLinkToNoteUrl, run_sel_s, generate_guid)
...
def on_message_content(option):
    ...
    out_info(f"笔记 【{note_id}】 评论量：{content_num}")
    time.sleep(1)
    ...
    while check_end():
        ...
        if items:
            for item in items:
                ...
                num += 1
                if num > max_num:
                    out_info(f"笔记 【{note_id}】 评论已经提取 {max_num} 条")
                    is_jump = True
                    break
                if not check_end():
                    is_jump = True
                ...
                try:
                    ...
                    out_info(f"{num}. 【{timestamp_to_date(create_time)}】 【{usre_name}】 评论：{content} 作者：{'是' if is_author else '否'} 点赞：{like} IP属地：{ip_location}")
